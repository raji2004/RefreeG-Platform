...
const Layout: React.FC<LayoutProps> = ({ profileImage, children }) => {
    ...
    useEffect(() => {
        if (
            ["/dashboard/MyCauses", "/dashboard/DonationHistory", "/signed-petitions"].includes(
                pathname
            )
        ) {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <div className="flex flex-1">
                ...
                {/* Mobile Sidebar */}
                <div
                    className={`fixed inset-0 z-40 transition-transform transform ${isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full"
                        } md:hidden`}
                >
                    {/* Sidebar panel */}
