...
type CampaignCardProps = {
  ...
  showProgress?: boolean; // Optional prop to show/hide progress bar
};

export const CampaignCard: React.FC<CampaignCardProps> = ({
  contractAddress,
  showProgress = false, // Default to false
}) => {
  ...
  return (
    <div className="flex flex-col justify-between max-w-sm p-6 bg-white border border-slate-200 rounded-lg shadow">
      <div>
        {showProgress && !isLoadingBalance && !isLoadingGoal && (
          <div className="mb-4">
      ...
      <Link
        href={`/crypto-donation/campaign/${contractAddress}`}
        passHref={true}
      >
